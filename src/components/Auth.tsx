...
import { Cpu, Zap, AlertCircle, Wifi } from 'lucide-react';
...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const [networkId, setNetworkId] = useState('main-net'); // Default network
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!username.trim() || !networkId.trim()) return;
    setError('');
    setLoading(true);
    ...
    setTimeout(() => {
      ...
      // Generate random initial resource profile for this session
      const initialCpu = Math.floor(Math.random() * 25) + 5; // 5-30%
      const initialMem = Math.floor(Math.random() * 40) + 10; // 10-50%
      ...
      if (isLogin) {
        ...
        if (db[username]) {
          ...
          // Reset runtime stats but keep points & update network preference
          const sessionUser: User = {
             ...existingUser,
             id: existingUser.id,
             networkId: networkId.trim(), // Update to current network selection
             mode: UserMode.IDLE, // Default to IDLE on login
             cpuUsage: initialCpu,
             memoryUsage: initialMem,
    ...
    <div className="flex items-center justify-center min-h-screen px-4">
      <div className="w-full max-w-md p-8 bg-slate-900/80 backdrop-blur-xl border border-cyan-500/30 rounded-2xl shadow-2xl shadow-cyan-500/20 relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2 font-mono flex items-center gap-2">
              <Wifi className="w-4 h-4 text-cyan-400"/> GRID_NETWORK_NAME
            </label>
            <input
              type="text"
              value={networkId}
              ...
              className="w-full bg-slate-800/50 border border-slate-700 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent outline-none transition-all font-mono placeholder-slate-600"
              placeholder="e.g. Office_Local, My_Test_Net"
              required
            />
            <p className="text-[10px] text-slate-500 mt-1">Both devices must use the same Network Name to see each other.</p>
          </div>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                {isLogin ? 'AUTHENTICATE & JOIN GRID' : 'INITIALIZE NEW NODE'}
              </>
            )}
          </button>
        </form>
